WITH base_events AS (
    /* 关卡难度分析拓展版（lv_name 分组）。
       以 lv_name 为唯一聚合键，lv_id 从 lv_name 提取首个 _ 前的数字（SPLIT_PART）。
       过滤：lv_name IS NOT NULL、排除已弃用的 Level_ 前缀。
       排序：默认按 lv_name，备选按 lv_id。 */
    SELECT
        e."#user_id",
...
, user_lv_base AS (
    ...
    GROUP BY e."#user_id", e.lv_name
)

/* 每关分位值：通关用户(1/0.90/0.99) + 未通关用户(0.90) */
, user_lv_quantile AS (
    SELECT
        MAX(b.lv_id) AS lv_id,
        b.lv_name,
...
, user_lv_agg AS (
    ...
    FROM user_lv_base b
    LEFT JOIN user_lv_quantile q ON b.lv_name = q.lv_name
    GROUP BY b.lv_name
)
...
SELECT
    ...
    ROUND(9999, 2) AS "关间流失率(%)",
    ROUND(9999, 2) AS "总流失率(%)",
...
FROM level_stats ls
LEFT JOIN user_lv_agg ua ON ls.lv_name = ua.lv_name
ORDER BY ls.lv_name
    -- 备选：按关卡数字ID排序 -- ORDER BY ls.lv_id
