...
| WorkBuddy（执行者） | 校验任务包；按 LG 当前规范和白名单目标创建或追加内容；检查锚点、幂等标记和链接；生成执行报告；把冲突任务转入待确认。 | 重新解释对话/代码；自行分类；移动/删除历史材料；改业务代码；写 TS/MT 项目目录；commit/push；处理无证据任务。 |
...
- WorkBuddy 仅可写 `/Users/dean/LibraryG` 中的上述运行目录、`01-DAILY/`、`02-PROJECTS/`、`03-KNOWLEDGE/`、`04-TEMPLATES/`；不得写 MT/TS 代码仓库。TS / MT 的项目内记忆同步只能作为报告项或待确认项，由 Codex 在用户授权后处理。
...
2. 读取 `AGENTS.md`、`AI总MOC.md`、`工作空间总纲.md`、[[02-PROJECTS/Agent/Memory|Agent Memory]]、本方案、[[02-PROJECTS/Agent/工作流/规范-任务知识沉淀闭环与自动巡检|闭环规范]]、[[02-PROJECTS/Agent/工作流/工作内容日志同步规范|Daily 规范]]。
...
| `knowledge-closure-daily-audit` | 每日 21:45，执行器之后 | 当天 Daily、当天报告、ready/needs-review 队列、当前 LG 规范入口 | 审计报告；仅补包中已允许的修复；检查核心规则是否统一 | 报告“已补 / 待确认 / 无证据”。 |
| `knowledge-closure-weekly-health` | 周日 22:00，晚于每日审计 | 最近 7 天 Daily、INBOX、近 7 天正式文档、近期规范/任务包/自动化说明 | 周报 + `needs-review` 任务包 | 只生成问题与明确链接修复包，不重写工作事实；发现规则冲突必须列出。 |

每日审计和周度健康巡检的检查清单以闭环规范为准。它们整理规范内容时必须同时检查规则是否统一，尤其是 LG 唯一源、MOC/MEMORY 逐级检索、Daily 三项目节、TS/MT 执行空间边界和自动化写入权限。它们不能直接把“发现文件更新”写成工作日志，只能报告线索或请求 Codex 预处理。
...
```text
工作目录固定为 /Users/dean/LibraryG。LG 是唯一知识库与规范源；TS / MT 只作为项目执行空间和事实来源。
先读取 AGENTS.md、AI总MOC.md、工作空间总纲.md、02-PROJECTS/Agent/Memory.md、.workbuddy/knowledge-closure/README.md、02-PROJECTS/Agent/工作流/方案-WorkBuddy日志知识闭环自动化实施.md、规范-任务知识沉淀闭环与自动巡检.md、工作内容日志同步规范.md。

只领取 .workbuddy/knowledge-closure/queue/ready/ 中按文件名最靠前的一份任务包。仅处理 frontmatter 同时满足 status: ready 与 risk: low 的任务包。
逐项检查 schema、来源、目标白名单、锚点唯一性、幂等标记和前置条件。仅执行任务包给出的完整正文；允许 daily_append、create_document、moc_append、codemap_append、report_only。禁止自行总结、归类、覆盖、移动、删除、写 TS/MT 项目目录、改项目代码、git commit/push。
...
```text
工作目录固定为 /Users/dean/LibraryG。LG 是唯一知识库与规范源；读取 AGENTS.md、AI总MOC.md、工作空间总纲.md、Agent Memory、闭环规范、Daily 同步规范和当天 WorkBuddy 执行报告。
检查当天 01-DAILY/YYYY-MM-DD.md 是否有今日索引及 TileMatch、TileScape、LibraryG 三节；检查 ready/needs-review 队列和当天已完成任务的验证结果。
同时检查 LG 核心规则是否统一：AGENTS.md、AI总MOC.md、工作空间总纲.md、Agent Memory、任务产出入库规范、闭环规范和自动化方案不能互相冲突；若发现旧规则与“LG 唯一知识库与规范源”冲突，只登记差异或生成 needs-review，不自行扩大权限。

不得根据 mtime、团队提交、构建产物或猜测补写个人工作。不得自行写知识正文；只有已有 ready 任务包的 action 才能写入。发现 TS / MT 项目内记忆或本地规范未同步 LG 时，只生成待确认项，不直接写项目目录。报告按“已补、待确认、无证据/风险”输出。
```

### C. `knowledge-closure-weekly-health`

```text
工作目录固定为 /Users/dean/LibraryG。LG 是唯一知识库与规范源；读取 AGENTS.md、AI总MOC.md、工作空间总纲.md、Agent Memory、闭环规范、Daily 同步规范和最近七天 WorkBuddy 报告。
检查最近七天 Daily 的日期断档、今日索引和三项目节；检查 00-INBOX 中超过七天的材料；检查近七天新增稳定文档是否含来源、状态、关联并从项目 MOC/专题索引/CodeMap 可达；检查近七天新增或修改的规范、任务包和自动化说明是否仍符合 LG 唯一源、逐级检索、Daily 分节、项目执行空间和自动化权限边界。
