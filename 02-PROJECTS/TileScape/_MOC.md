...
| 元素牌 / 磁铁牌持续任务 | [[03-KNOWLEDGE/Game-Logic/任务-元素牌开发|磁铁牌与元素牌开发任务]]：TS 当前实现与 Trial 验证，MT 仅作历史机制对照 |
...
- [[02-PROJECTS/Agent/工作流/规范-多项目工作流与复现|多项目工作流与复现]] — MT / TS 同源代码下的记录、对照与复现闭环

## 游戏逻辑

> [!important] 当前持续主线（用户确认，2026-09-23）
> 1. [[02-PROJECTS/TileScape/游戏逻辑/任务-买量承接与章节拆分长期任务|买量承接与章节拆分]]：广告素材到首关、Chapter0 与章节体验的产品方案及研发验证入口。
> 2. [[03-KNOWLEDGE/Game-Logic/任务-元素牌开发|磁铁牌与元素牌开发]]：磁铁牌的编辑器放置、局内行为与 Trial 验证；MT 只提供历史对照。
> 3. [[02-PROJECTS/TileScape/游戏逻辑/任务-DLC全链路梳理与维护|DLC 全链路梳理与维护]]：启动、Home、画廊、解锁、Endless 与活动资源链路。
>
> [[02-PROJECTS/Agent/工作流/方案-WorkBuddy日志知识闭环自动化实施|WorkBuddy 工作模式]] 是独立的知识落盘流程，不属于上述任何一条产品任务；磁铁牌仅曾作为其小样本任务包。

- [[02-PROJECTS/TileScape/游戏逻辑/设计-TS买量承接方案-2026-09-16.html|TS 买量承接方案（视觉方案页）]] — 基于 Royal Match、Royal Kingdom、Tile Family 的公开资料；广告素材至长期循环的承接链，以及“剧情衔接 / 特殊任务系统”两种方案与验证边界。对话整理与外部公开资料，待结合 TS 当前首屏/关卡/活动实现确认。原 Canvas 仅保留为编辑索引，不建议用于阅读。

- [[02-PROJECTS/TileScape/游戏逻辑/任务-买量承接与章节拆分长期任务|买量承接与章节拆分长期任务]] — 长期主线：片头视频为第一阶段，记录方案 owner / 程序交付 / 本地探索的边界、当前代码基线、问题、验证状态以及与 Chapter0 拆分方案的关系。

- [[02-PROJECTS/TileScape/游戏逻辑/分析-DDAV3场景反例与迭代选项-2026-09-13|DDAV3 场景反例与迭代选项]] — `DDA-V3-Test@cd3` 的历史静态矩阵；现行适用性见旧文档核对。

- [[02-PROJECTS/TileScape/游戏逻辑/任务-DDAV3逻辑策略持续研究|DDAV3 逻辑策略持续研究]] — 六个研究目标、版本增量、证据状态与自动化续跑断点。

- [[02-PROJECTS/TileScape/游戏逻辑/更新-DDAV3-dev逻辑与策略评估-2026-09-20|DDAV3 在 dev 的逻辑与策略评估]] — `dev@842093623` 的独立 V3 策略、回放/曲线/观测能力、风险与运行验证入口。

- [[02-PROJECTS/TileScape/游戏逻辑/核对-DDAV3旧文档与dev实现-2026-09-20|DDAV3 旧文档与 dev 实现核对]] — 旧评估、反例矩阵和持续主档在新版实现下的失效、保留与待验证项。

- [[02-PROJECTS/TileScape/游戏逻辑/评估-DDAV3逻辑与实现策略-2026-09-11|DDAV3 逻辑与实现策略评估]] — `DDA-V3-Test@9be` 的历史基线；当前 dev 请使用 9 月 20 日评估与核对文档。

- [[02-PROJECTS/TileScape/游戏逻辑/任务-DLC全链路梳理与维护|DLC 全链路梳理与维护]] — DLC 唯一长期主档：启动/Home/画廊/解锁/Endless/活动全链路、当前配置、差距与验收入口
