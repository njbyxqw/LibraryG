...
| 编号 | 目标 | 完成标准 | 当前状态 |
|---|---|---|---|
| G1 | 当前实现基线 | 入口、配置、揭牌、指标、候选、执行、统计和记录链路有源码依据，核对版本差异 | `dev@842093623` 的独立 V3 链路已静态更新；等待实际启用与测试执行验证 |
...
| G5 | 迭代验证基础 | 配置、算法版本、回放、日志、测试和性能缺口及最小补齐项齐全 | 当前 dev 已有参数快照、回放、日志和 EditMode 测试源码；等待实际测试、性能与 Layer A/B 执行 |
| G6 | 分阶段路线 | 以依赖、收益、风险排序，写清每阶段范围、验证与退出条件 | 静态产出完成：三条路线、依赖与退出标准已记录；未实施 |

研究文档完成与算法运行有效性分别登记。R16 完成了旧 V3 静态研究；R18 发现当前 dev 已具备独立 V3 和测试源码，故任务重新进入运行验证阶段。只有六项目标对应产出齐全、关键未决项已说明依据/影响/处理方式，才能结束本研究；不能因已有摘要或达到某个运行窗口而标完成。若关键结论依赖运行数据而暂不可得，保留证据不足状态，不虚构结果。

## 当前版本与事实

> [!info] 现行基准
> 运行和策略判断以 `dev@842093623` 为准，详见 [[02-PROJECTS/TileScape/游戏逻辑/更新-DDAV3-dev逻辑与策略评估-2026-09-20|dev 逻辑与策略评估]] 与 [[02-PROJECTS/TileScape/游戏逻辑/核对-DDAV3旧文档与dev实现-2026-09-20|旧文档核对]]。下列内容在 R18 以前均是历史版本记录。
...
### R18：dev 重新合入独立 V3，并进入验证阶段（2026-09-20）

- 版本：`dev@842093623`。本轮静态核对显示 `V3/` 目录、`DDAV3RecordConfig`、`DDAV3BaseCurve`、V3 EditMode 测试族均在当前树中；R17 针对 `dev@34f709532` 的“无 V3 控制器”只保留为历史记录，不再代表当前 dev。
- 当前链路：`TileMatchGame` 解析运行时/自定义数据的版本字符串，`InLevelDDASelection` 对新局支持 `V3` 且对回放恢复录像类型；`TileMatchGameLogic` 用关卡 `ddaCurve`（Bot 可覆盖）建 V3 Config，`InLevelDDA` 再实例化 `InLevelDDAV3Strategy`。回放配置包含曲线点、成本、DeadBand、初始棋盘数和分配策略，解决旧版参数随版本漂移的主要风险。
- 策略：以 `HardSlack = 容量 - 未成组数 - FastNcd` 跟踪曲线目标；带内不调控，带外在未见隐藏合法候选中找可严格改善目标距离的类型，再随机选实体。安全候选优先；无安全路线仍可能产生 `UnsafeFallback`，无改善则不换牌。OverBar 和共享遮挡已纳入压力路径。
- 新增证据文档：[[02-PROJECTS/TileScape/游戏逻辑/更新-DDAV3-dev逻辑与策略评估-2026-09-20|dev 逻辑与策略评估]]。该文取代“当前 dev 不含 V3”的操作判断；旧 `DDA-V3-Test@cd3aea166` 仍只作为历史对照。
- 验证边界：本轮未运行 Unity/EditMode，未验证曲线导表和实际 V3 启用覆盖率。下一阶段从静态研究转为测试执行、固定 seed 回放和线上/机器人指标验证；重点监控 `UnsafeFallback`、`NoImprovement`、候选实体拓扑误差和候选评估耗时。
